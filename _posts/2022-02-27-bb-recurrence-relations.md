...
Note that this analysis is similar to [Pascal Michel's analysis](https://bbchallenge.org/~pascal.michel/beh.html#tm62h) of Pavel Kropitz's current `BB(6, 2)` record holder which similarly seems to have a 2 level recurrence behavior and thus leads to this sort of exponential recurrence (<code>C(9k + 9) -> C((50 * 4<sup>k+1</sup> - 11)/3)</code>). Sadly, unlike in that case, for our configuration `g` we will never have the possibility to iterate `g` more than 3 times and thus this machine cannot be modified in the way that "Wythagoras" did to create the current BB(7,2) champion.
